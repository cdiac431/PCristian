<div class="modal fade" id="finProjecte" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title text-center">Finalitzar Projecte</h3>
        </div>
        <div class="modal-body">
          <form method="POST" id="formeliminar" action="{{route('projectes.final',$projecte->id)}}">
            @csrf
            @method('post') 
            <input id="id_projecte" name="id_projecte" type="hidden" value="{{$projecte->id}}">
            <p>Estas segur que vols finalitzar <strong>{{$projecte->nom_projecte}}</strong>?</p>
